function addToCart(product_id, type, page = "na", quantity = 0) {
    ...
    if ($("#variation").length) {
        if ($("#variation").val() === "") {
            return toastr.error("Select a variation first");
        }
    }

    if ($("#color").length) {
        if ($("#color").val() === "") {
            return toastr.error("Select a color first");
        }
    }
    ...
    $.ajax({
        ...
        headers: {
            ...
            Authorization: "Bearer " + csrfToken, // Include the CSRF token as Authorization Bearer
            // Other headers...
        },
        dataType: "json",
...
function cartUpdate(cart_id, type) {
    let total;
    let quantity = Number(
        document.getElementById("current_quantity_" + cart_id).innerHTML
    );
